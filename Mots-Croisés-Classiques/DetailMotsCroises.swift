...
class DetailMotsCroises: UITableViewController, GADBannerViewDelegate {
    ...
    func showAlert4 () {
        ...
        let alert = UIAlertController(title: "Mots Croisés Classiques", message: "Votre avis est important pour améliorer l'application. Vous aimez, vous n'aimez pas? Ça m'intéresse!", preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "D'accord, je donne mon avis", style: UIAlertAction.Style.default, handler:{(alert: UIAlertAction!) in self.rateApp(appId: "id1210494247") { success in
            print("RateApp \(success)")
            }}))
        alert.addAction(UIAlertAction(title: "Pas Maintenant", style: UIAlertAction.Style.default, handler: nil))
        //self.present(alert, animated: true, completion: nil)
        alert.addAction(UIAlertAction(title: "Ne plus me montrer cette fenêtre", style: UIAlertAction.Style.default, handler: {(alert: UIAlertAction!) in self.fenetre = true; UserDefaults.standard.set(self.fenetre, forKey: "fenetre") }))
        self.present(alert, animated: true, completion: nil)
    }
    ...
    @IBAction func unwindToVC(segue: UIStoryboardSegue) {
        ...
        self.tableView.reloadRows(at: [[0, index] ], with: UITableView.RowAnimation.none)
        UserDefaults.standard.set(stateOfMotsCroises, forKey: "stateOfMotsCroises")
        if fenetre == false {
            showAlert4()
        }
